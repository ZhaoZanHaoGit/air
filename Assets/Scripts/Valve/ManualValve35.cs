...
public class ManualValve35 : BaseValve
{
    ...
    // ports[4]:z  P口与B口相通，A口与R口相通，S口截止
    // ports[5]: z1 进气口P口与A口相通，B口与S口相通，R口截止。
    ...
    public override void ProcessLogic()
    {
        ...
        if (ports[4].pressure > 0.1f)
        {
            SetValveState(-1);
        }
        // 只要 Z1 有压力，阀芯就推向左侧（P-B导通），并保持
        else if (ports[5].pressure > 0.1f)
        {
            SetValveState(1);
        }
        else if (ports[5].pressure == 0 && ports[4].pressure == 0)
        {
            SetValveState(0);
        }
