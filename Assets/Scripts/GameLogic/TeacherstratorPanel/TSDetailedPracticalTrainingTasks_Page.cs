...
public class TSDetailedPracticalTrainingTasks_Page : BasePage
{
    public override PageUIType GetUIType()
    {
        return PageUIType.TSDetailedPracticalTrainingTasks_Page;
    }
    ...
    private void UpdateLearnDatas()
    {
        _ = NetHelper.Instance.GetSoftLearnDatasByClasses(AppController.Instance.softData, (datas) =>
        {
            ...
            if (_datas != null && _datas.Count > 0)
            {
                ...
                ChartHelper.UpdateChart_AllUserCount<EVSType>(_datas,_sxAssessmentSerie, _sxAssessmentChart, LearnType.Score, EVSType.None);
                ChartHelper.UpdateChart_AllUserCount<EVSType>(_datas, _sxAssessmentTimeSerie, _sxAssessmentTimeChart, LearnType.LearnTime, EVSType.None);
            }
            else
            {
                OnResetView();
    ...
    private void InitLearnDatas()
    {
        ...
        else
        {
            ...
            if (datas != null && datas.Count > 0)
            {
                ...
                ChartHelper.UpdateChart_AllUserCount<EVSType>(datas,_sxAssessmentSerie, _sxAssessmentChart, LearnType.Score, EVSType.None);
                ChartHelper.UpdateChart_AllUserCount<EVSType>(datas,_sxAssessmentTimeSerie, _sxAssessmentTimeChart, LearnType.LearnTime, EVSType.None);
            }
            else
            {
                OnResetView();
        ...
        for (int i = 0; i < datas.Count; i++)
        {
            for (int j = 0; j < courseIds.Count; j++)
            {
                if (datas[i].CourseID == courseIds[j])
                {
                    ...
                    learningNotesItem.Init(sequenceNumber, datas[i], Gamemode.KaoHe);
                    sequenceNumber++;
                }
            }
        }
