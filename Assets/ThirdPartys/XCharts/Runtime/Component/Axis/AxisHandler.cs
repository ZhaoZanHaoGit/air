    ...
    where T : Axis
    {
        ...
        internal void UpdateAxisTickValueList(Axis axis)
        {
            ...
            else if (axis.IsValue())
            {
                ...
                if (!Runtime.ChartHelper.IsEquals(axis.context.maxValue, list[list.Count - 1]))
                {
            ...
            float axisStartX, float axisStartY, float axisLength, float relativedLength)
        {
            ...
            var axisObj = Runtime.ChartHelper.AddObject(objName,
                chart.transform,
            ...
            Runtime.ChartHelper.HideAllObject(axisObj);
            ...
            for (int i = 0; i < splitNumber; i++)
            {
                ...
                var label = Runtime.ChartHelper.AddAxisLabelObject(splitNumber, i,
                ...
                if (orient == Orient.Horizonal)
                {
                    ...
                    switch (axis.axisName.labelStyle.position)
                    {
                        case LabelStyle.Position.Start:
                            ...
                            label = Runtime.ChartHelper.AddChartLabel(s_DefaultAxisName, axisObj.transform, axis.axisName.labelStyle,
            ...
            Orient orient, float startX, float startY, float axisLength)
        {
            ...
            if (AxisHelper.NeedShowSplit(axis))
            {
                ...
                {
                    size += 1;
                    if (!Runtime.ChartHelper.IsEquals(axis.GetLastLabelValue(), axis.context.maxValue))
                        size += 1;
                }
